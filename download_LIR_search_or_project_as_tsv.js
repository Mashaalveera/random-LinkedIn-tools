async function initSearchResultsDownloader(){
var reg = (o, n) => o ? o[n] : '';
...
async function loopThroughSearchRes() {
  ...
  var totalSearchRes = cn(document,'talent-pool-tab selected')[0] ? parseInt(cn(document,'talent-pool-tab selected')[0].innerText.replace(/\D+/g,'')) : 0;
  var marker = totalSearchRes > 1000 ? 1000 : totalSearchRes;
  ...
  if (numRes > 24) {
    for (var i = 25; i < 1000; i=i+25) {
      gi(document, 'searchResultsDL_cont').innerText = (Math.round((i/marker)*10000)/100) + '% complete';
  ...
  downloadr(temp_csv_dl,'SearchResults_'+reg(/\d+/.exec(window.location.href),0)+'.tsv');
  gi(document, 'searchResultsDL_cont').outerHTML = '';
}

async function getSearchResultsAsTSV(){
  var cont = ele('div');
  ...
  var totalSearchRes = cn(document,'talent-pool-tab selected')[0] ? parseInt(cn(document,'talent-pool-tab selected')[0].innerText.replace(/\D+/g,'')) : 0;
  var marker = totalSearchRes > 1000 ? 'the first 1000 search results.' : totalSearchRes + ' search results';
  cont.innerText = 'Downloanding ' + marker;
  await loopThroughSearchRes(); 
}
getSearchResultsAsTSV();
}
...
if(/linkedin.com\/recruiter\/projects\/\d+/.test(window.location.href)) initProjectDownloader();
if(/linkedin.com\/recruiter\/smartsearch/.test(window.location.href)) initSearchResultsDownloader();
